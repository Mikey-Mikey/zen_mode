...
hook.Add( "ShouldCollide", "ZenMode_ResolveCollisions", function( ent1, ent2 )
    ...
    if ent1:IsPlayer() and not ent2:IsPlayer() and ( ent1:GetZenMode() or IsOwnerZen( ent2 ) ) and IsValid( CPPIGetTopOwner( ent2 ) ) and CPPIGetTopOwner( ent2 ) ~= ent1 then
        return false
    end

    -- Entity vs Player collision
    if ent2:IsPlayer() and not ent1:IsPlayer() and ( ent2:GetZenMode() or IsOwnerZen( ent1 ) ) and IsValid( CPPIGetTopOwner( ent1 ) ) and CPPIGetTopOwner( ent1 ) ~= ent2 then
        return false
    end
end )
